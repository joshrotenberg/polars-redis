//! Cluster-aware batch iterator for Redis list data.
...
use super::reader::{ListData, fetch_lists_cluster};
use crate::cluster::DirectClusterKeyScanner;
use crate::error::{Error, Result};
use crate::io::types::hash::BatchConfig;

/// Cluster-aware iterator for scanning Redis lists.
pub struct ClusterListBatchIterator {
...
impl ClusterListBatchIterator {
    /// Create a new cluster list batch iterator.
    pub fn new(nodes: &[impl AsRef<str>], schema: ListSchema, config: BatchConfig) -> Result<Self> {
    ...
    fn fetch_batch(&mut self, keys: &[String]) -> Result<Vec<ListData>> {
        ...
        self.runtime.block_on(fetch_lists_cluster(&mut conn, keys))
    }
