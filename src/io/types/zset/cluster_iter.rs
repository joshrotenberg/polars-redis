//! Cluster-aware batch iterator for Redis sorted set data.
...
use super::reader::{ZSetData, fetch_zsets_cluster};
use crate::cluster::DirectClusterKeyScanner;
use crate::error::{Error, Result};
use crate::io::types::hash::BatchConfig;

/// Cluster-aware iterator for scanning Redis sorted sets.
pub struct ClusterZSetBatchIterator {
...
impl ClusterZSetBatchIterator {
    /// Create a new cluster sorted set batch iterator.
    pub fn new(nodes: &[impl AsRef<str>], schema: ZSetSchema, config: BatchConfig) -> Result<Self> {
    ...
    fn fetch_batch(&mut self, keys: &[String]) -> Result<Vec<ZSetData>> {
        ...
        self.runtime.block_on(fetch_zsets_cluster(&mut conn, keys))
    }
